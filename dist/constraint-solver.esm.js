import { Solver, Strength, Variable, Constraint, Operator } from 'kiwi.js';
...
function peg$parse(input, options) {
  ...
  }, ".", peg$literalExpectation(".", false), function (digits, subs) {
...
function compile(input) {
  ...
  var _c = function _c(expr) {
    ...
    switch (expr.type) {
      case 'Inequality':
        var op = expr.operator == '<=' ? Operator.Le : Operator.Ge;
        var i = new Constraint(_c(expr.left), op, _c(expr.right), strength$1[expr.strength.toLowerCase()]);
        solver.addConstraint(i);
        return i;

      case 'Equality':
        var i = new Constraint(_c(expr.left), Operator.Eq, _c(expr.right), strength$1[expr.strength.toLowerCase()]);
        solver.addConstraint(i);
        return i;
      ...
      case 'EditableVariable':
        if (!vars[expr.name]) {
          vars[expr.name] = new Variable(expr.name);
      ...
      case 'Variable':
        if (!vars[expr.name]) vars[expr.name] = new Variable(expr.name);
        return vars[expr.name];
    }
  };
