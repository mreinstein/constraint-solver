...
function operatorLookup(str) {
  if (str === '>=') return Operator.Ge;
  if (str === '<=') return Operator.Le;
...
function irExpressionToKiwi(identifiers, irExpression) {
  if (irExpression.type === 'IDENTIFIER') {
    if (!identifiers[irExpression.value]) identifiers[irExpression.value] = new Variable(irExpression.value);
  ...
  try {
    for (var _iterator = irExpression.arguments[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      if (argument.type === 'IDENTIFIER') {
        if (!identifiers[value]) identifiers[value] = new Variable(value);
...
function irConstraintToKiwi(identifiers, irConstraint) {
  ...
  return new Constraint(irExpressionToKiwi(identifiers, irConstraint.lhs), operatorLookup(irConstraint.operator.value), irExpressionToKiwi(identifiers, irConstraint.rhs), strengthLookup(irConstraint.strength));
}
...
function constraints() {
  ...
  editableVariables.forEach(function (editVariable) {
    if (!identifiers[editVariable.name]) {
      var v = new Variable(editVariable.name);
