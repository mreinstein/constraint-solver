...
export default function compile (input) {
	...
	const _c = function (expr) {
		if (exprs[expr])
			return exprs[expr]

		switch(expr.type) {
		    case 'Inequality':
		      const op = (expr.operator == '<=') ? kiwi.Operator.Le : kiwi.Operator.Ge
		      const ineq = new kiwi.Constraint(_c(expr.left), op, _c(expr.right), strength[expr.strength.toLowerCase()])
		      solver.addConstraint(ineq)
		      return ineq

		    case 'Equality':
		      const eq = new kiwi.Constraint(_c(expr.left), kiwi.Operator.Eq, _c(expr.right), strength[expr.strength.toLowerCase()])
		    ...
		    case 'MultiplicativeExpression':
		      if (expr.operator == '*') {
			    ...
			    const lhs = expr.right.type === 'NumericLiteral' ? _c(expr.left) : _c(expr.right)
			    const rhs = expr.right.type === 'NumericLiteral' ? _c(expr.right) : _c(expr.left)
